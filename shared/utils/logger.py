...
class ServiceLogger:
    ...
    def __init__(self, service_name: str):
        ...
        # File handler - use project root logs directory
        # Find project root (go up until we find logs/)
        current = Path.cwd()
        log_dir = None
        for parent in [current] + list(current.parents):
            potential_log_dir = parent / "logs"
            if potential_log_dir.exists() or (parent / "requirements.txt").exists():
